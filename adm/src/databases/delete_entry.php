...
if (isset($_GET['id'])) {
    ...
    $stmt = $con->prepare("Update tbl_milk_records set val_status=2 WHERE m_id = ?");
    $stmt->bind_param("i", $m_id);

    // Execute the statement
    if ($stmt->execute()) {
        // Record deleted successfully
        echo "<script>alert('Milk details on Pending status!'); window.location.href='../html/milk_edit.php';</script>";
    } else {
        // Error occurred
